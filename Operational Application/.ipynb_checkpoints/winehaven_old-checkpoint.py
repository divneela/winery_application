...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QPushButton, QVBoxLayout, QHBoxLayout, QCheckBox, QMessageBox, QListWidget, QListWidgetItem, QScrollArea, QDialog, QInputDialog, QSpacerItem, QSizePolicy, QTableWidget, QTableWidgetItem, QAbstractItemView, QHeaderView, QFormLayout, QComboBox, QGroupBox, QDateEdit, QTextEdit, QGridLayout, QDoubleSpinBox, QSpinBox, QMainWindow, QTabWidget
...
class SignUp(QDialog):
    ...
    def submit_form(self):
        ...
        new_customer_id = 1000 + customer_count + 1
...
#         self.reservation_page = ReservationPage(self)
#         self.reservation_tab_layout.addWidget(self.reservation_page)        
        
         self.cart_page = CartPage(self, wine_ids)
...
class WishlistPage(QWidget):
    def __init__(self, username, parent=None):
        ...
        for wine_id in wine_ids:
           ...
           # name_label = QLabel(wine_name)
            ...
            remove_button.clicked.connect(lambda checked, wine_id=wine_id: self.remove_from_cart(wine_id))
    ...
    def remove_from_cart(self, wine_id):
...
class ViewProductsPage(QDialog):
    def __init__(self, username, parent=None, result=None):
        ...
        # Add the button to the layout
        layout.addWidget(filter_button, alignment=Qt.AlignCenter)

        # Connect the button to the filter_products function
...
class ReviewsPage(QWidget):
    def __init__(self, username, parent=None, result=None):
        ...
        self.feedback_table.setHorizontalHeaderLabels(['Rating', 'Feedback', 'PurchaseExperience', 'Username'])
        self.feedback_table.horizontalHeader().setFixedHeight(40)
        self.feedback_table.horizontalHeader().setSectionResizeMode(QHeaderView.Fixed)
        self.feedback_table.horizontalHeader().setStretchLastSection(True)


        # Add the feedback table to a scroll area
        scroll_area = QScrollArea()
        scroll_area.setWidgetResizable(True)
        scroll_area.setWidget(self.feedback_table)
        layout.addWidget(scroll_area)
    ...
    def show_feedback(self):
        ...
        for i, row in enumerate(feedback):
            if len(row) >= 4:
                ...
                rating_item = QTableWidgetItem(str(row[0]))
                feedback_item = QTableWidgetItem(row[1])               
                purchase_rating_item = QTableWidgetItem(str(row[2]))
                username_item = QTableWidgetItem(row[3])
                self.feedback_table.setItem(i, 0, rating_item)
                self.feedback_table.setItem(i, 1, feedback_item)     
                self.feedback_table.setItem(i, 2, purchase_rating_item)
                self.feedback_table.setItem(i, 3, username_item)
            else:
                print(f"Skipping row {i} due to insufficient elements.")         
            
        # Set column widths
        self.feedback_table.setColumnWidth(0, 50)
        self.feedback_table.setColumnWidth(1, 800)
        self.feedback_table.setColumnWidth(2, 130)
        self.feedback_table.setColumnWidth(3, 100)
        self.feedback_table.horizontalHeader().setStretchLastSection(True)
